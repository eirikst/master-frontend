 ...
 */
public class GetMyUpdatedLogTask extends AsyncTask<Void, Void, Pair<Integer, ResponseEntity<String>>> {
    public interface OnFinishedLoadingMyUpdatedLogListener {
        void onLoadingMyUpdatedLogSuccess(JSONArray user);
        void onLoadingMyUpdatedLogFailure(int code);
    }

    private String tag = getClass().getSimpleName();

    private OnFinishedLoadingMyUpdatedLogListener listener;
    private int lastLogId;

    public GetMyUpdatedLogTask(OnFinishedLoadingMyUpdatedLogListener listener, int lastLogId) {
    ...
    protected Pair<Integer, ResponseEntity<String>> doInBackground(Void... params) {
        ...
        try {
            response = template.exchange(Constants.BACKEND_URL + "me/log/update?lastLogId=" + lastLogId,
    ...
    protected void onPostExecute(Pair<Integer, ResponseEntity<String>> response) {
        if (response.first == Constants.OK) {
            ...
            try {
                ...
                listener.onLoadingMyUpdatedLogSuccess(log);
            }
            catch(JSONException e) {
                Log.w(tag, "JSON error:" + e);
                listener.onLoadingMyUpdatedLogFailure(Constants.JSON_PARSE_ERROR);
            }

        }
        else {
            listener.onLoadingMyUpdatedLogFailure(response.first);
        }
    }
}